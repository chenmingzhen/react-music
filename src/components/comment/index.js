...
class Comment extends React.PureComponent {
                    ...
                    <div>
                        {
                            comments.map((item, index) => {
                                return (
                                    <AntComment key={index}
                                                actions={[<span onClick={
                                                    () => {
                                                        this.likeOrDislikeComment(item, item.commentId)
                                                    }
                                                }>
                                                    {createElement(item.liked === true ? LikeFilled : LikeOutlined)}
                                                ...
                                                datetime={
                                                    timestampToTime(item.time, true)
                                                }/>
                                )
                            })
                        }
    ...
    render() {
        ...
        return (<div className={"comment-wrapper"}>
            {
                offset === 0 && this.renderHotComments(commentData.hotComments, "精选评论")
            }
            {this.renderHotComments(commentData.comments, "最新评论")}
            {this.renderPagination()}
            {
                (commentData.comments && commentData.comments.length === 0) ? <div>暂无评论</div> : ''
            }
        </div>)
    }
}
