...
class PlayList extends React.Component {
    ...
    getData() {
        getPlayList(this.props.match.params.id).then((data) => {
            this.setState(() => ({listData: data.playlist, id: this.props.match.params.id}));
    ...
    shouldComponentUpdate(nextProps, nextState, nextContext) {
        if (nextProps.match.params.id === this.state.id && this.state.listData.id && nextState.status === this.state.status) {
            return false
        }
    ...
    render() {
        ...
        if (data.creator && String(data.id) === this.props.match.params.id) {
            return (
                <div className={"self-playlist-wrapper"}>
                    <div className={"detail-wrapper"}>
                        ...
                        <div className="text-wrapper">
                            ...
                            <div className="creator-wrapper">
                                ...
                                <div className="time">{timestampToTime(data.createTime)}创建</div>
                            </div>
                            <div className="play-all">播放全部</div>
                            <div className="tag">{data.tags.map((item, index) => {
                                return item + "/"
                            })}</div>
                            <div
                                className="count">
                    ...
                    <div className={"select-wrapper"}>
                        <div className={this.state.status === 1 ? 'song active' : 'song'} onClick={() => {
                            this.setState({status: 1})
                        }}>歌曲列表
                        </div>
                        <div
                            className={this.state.status === 2 ? 'comment active' : 'comment'} onClick={() => {
                            this.setState({status: 2})
                        }}>评论({data.commentCount})
                        </div>
                    </div>
                    {
                        this.state.status === 1 ? (<SongList songList={this.state.listData.tracks}/>) : (
            ...
            return (<div style={{
                ...
                width: '100%',
                height: '100%'
            }}><Spin tip={"loading"} indicator={antIcon}/></div>)
        }
